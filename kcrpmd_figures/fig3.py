...
ax1.plot(s_ar, newFs_low - Fgs_low, color='g', label='new', dashes=[4, 2], zorder=2)
ax1.plot(s_ar, oldFs_low - Fgs_low, color='darkorange', label='ori', zorder=1)
#ax2.plot(s_ar, Fgs_high, color='k', label='adiabatic', dashes=[4, 1], zorder=4)
ax2.plot(s_ar, newFs_high - Fgs_high, color='g', label='new', dashes=[4, 2], zorder=3)
ax2.plot(s_ar, oldFs_high - Fgs_high, color='darkorange', label='ori', zorder=2)
ax2.plot(s_ar, oldFsa1_high - Fgs_high, color='blueviolet', label='ori*', zorder=1)
ax1.set_xlim(-4.7,4.7)
ax1.set_ylim(-0.0015,0.0028) #ax1.set_ylim(-0.002,0.03)
ax2.set_ylim(-0.0015,0.0028) #ax2.set_ylim(-0.002,0.03)
ax1.set_yticks([-0.001, 0.001]) #ax1.set_yticks([0.0, 0.01, 0.02])
ax2.set_xticks([-4, -2, 0, 2, 4])
ax2.set_yticks([-0.001, 0.001]) #ax2.set_yticks([0.0, 0.01, 0.02])
ax1.set_yticklabels([-0.001, 0.001]) #ax1.set_yticklabels(["$-10^{-3}$", " ", "$10^{-3}$"])
ax2.set_yticklabels([-0.001, 0.001]) #ax2.set_yticklabels(["$-10^{-3}$", " ", "$10^{-3}$"])
ax2.set_xlabel(r"$s$")
ax1.set_ylabel(r"$F^{\mathrm{KC}}(s)-F_{g}(s)$", labelpad=1)
ax2.set_ylabel(r"$F^{\mathrm{KC}}(s)-F_{g}(s)$", labelpad=1)
ax1.set_title("")
