...
class KcrpmdTst:
    ...
    def Fgs(self, s):
        ...
        exp_shift = np.max(exp_arg) - 500.
        return -np.log(np.trapezoid(np.exp(exp_arg - exp_shift), q_ar, axis = 0)) / self.beta - exp_shift / self.beta
    ...
    def Fsq(self, s, q):
        ...
        exp_shift = np.maximum(np.maximum(exp0_arg, exp1_arg), expKP_arg) - 500.
    ...
    def Fq(self, q):
        ...
        exp_shift = np.max(exp_arg) - 500.
        return -np.log(np.trapezoid(np.exp(exp_arg - exp_shift), s_ar, axis = 1)) / self.beta - exp_shift / self.beta
