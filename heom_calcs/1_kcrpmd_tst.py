...
print(f'beta = {beta}')
print(f'Omega = {ws}')
print(f'Gamma = {4.18608 / 1836.0}')
print(f'beta*hbar*Omega = {beta*ws}')
print(f'beta*hbar*Gamma ={beta * 4.18608 / 1836.0}')
Kq = lambda q: np.full_like(q, K0)
Vq = lambda q: 0.5 * mq * wq**2 * q**2
kcrpmd_tst = KcrpmdTst(beta, a, b, c, ms, ws, s0, s1, eps, Kq, Vq)
print()
print(f'Lambda = {kcrpmd_tst.lam}')
print(f'beta*Lambda = {beta * kcrpmd_tst.lam}')
exit()
###########################################################################################
