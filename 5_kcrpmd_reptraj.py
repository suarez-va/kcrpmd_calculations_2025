...
pref_pt1 = F"_reptraj_pt1"
pref_pt2 = F"_reptraj_pt2"
...
p_pt1 = [np.random.normal(scale = np.sqrt(mass[i] / beta)) for i in range(len(mass))]
p_pt2 = [-p for p in p_pt1]

init_nucl_pt1 = {"q":q, "p":p_pt1, "mass":mass, "force_constant":[0] * len(q), "init_type":0,
                 "ntraj":control_params["ntraj"], "ndof": len(q)}

init_nucl_pt2 = {"q":q, "p":p_pt2, "mass":mass, "force_constant":[0] * len(q), "init_type":0,
                 "ntraj":control_params["ntraj"], "ndof": len(q)}
...
if "use_kcrpmd" in control_params:
    ...
    py_pt1 = np.random.normal(scale = np.sqrt(my / beta))
    py_pt2 = -py_pt1

    init_elec_pt1 = {"init_type":0, "nstates":control_params["nstates"], "istate":0,
                     "rep":1, "ntraj":control_params["ntraj"],
                     "ndia":control_params["nstates"], "nadi":control_params["nstates"],
                     "y_aux_var":[y], "p_aux_var":[py_pt1], "m_aux_var":[my]}

    init_elec_pt2 = {"init_type":0, "nstates":control_params["nstates"], "istate":0,
                     "rep":1, "ntraj":control_params["ntraj"],
                     "ndia":control_params["nstates"], "nadi":control_params["nstates"],
                     "y_aux_var":[y], "p_aux_var":[py_pt2], "m_aux_var":[my]}
else:
    init_elec_pt1 = {"init_type":0, "nstates":control_params["nstates"], "istate":0,
                     "rep":1, "ntraj":control_params["ntraj"],
                     "ndia":control_params["nstates"], "nadi":control_params["nstates"]}
...
control_params.update({ "prefix":pref_pt1, "prefix2":pref_pt1 })
res_pt1 = tsh_dynamics.generic_recipe(control_params, kcrpmd_system_bath, model_params, init_elec_pt1, init_nucl_pt1, rnd_pt1)

control_params.update({ "prefix":pref_pt2, "prefix2":pref_pt2 })
res_pt2 = tsh_dynamics.generic_recipe(control_params, kcrpmd_system_bath, model_params, init_elec_pt2, init_nucl_pt2, rnd_pt2)
