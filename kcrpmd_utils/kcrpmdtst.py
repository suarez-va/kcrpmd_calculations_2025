...
class KcrpmdTst:
    ...
    def kGR(self):
        ...
        Kq_ar = self.Kq(q_ar)
        Pq_ar = self.Pq(q_ar)
        kMT_ar = 2 * np.pi * Kq_ar**2 * np.sqrt(self.beta / (4 * np.pi * self.lam)) * np.exp(-self.beta * (self.lam + self.eps)**2 / (4 * self.lam)) * Pq_ar
        return np.trapezoid(kMT_ar, q_ar)

    def kZUS(self, tauL):
        q_ar = self.q_array()
        Kq_ar = self.Kq(q_ar)
        Pq_ar = self.Pq(q_ar)
        kZUS_ar = (2 * np.pi * Kq_ar**2 * np.sqrt(self.beta / (4 * np.pi * self.lam)) * np.exp(-self.beta * (self.lam + self.eps)**2 / (4 * self.lam)) / (1 + 4 * np.pi * Kq_ar**2 * tauL / self.lam)) * Pq_ar
        return np.trapezoid(kZUS_ar, q_ar)
    ...
    def kIF(self, kBO, kBO_0):
        ...
        return np.trapezoid(kMT_ar * kBO / (kMT_ar + kBO_0), q_ar)
