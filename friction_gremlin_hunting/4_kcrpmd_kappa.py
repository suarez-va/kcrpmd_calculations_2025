...
for i, d in enumerate(itraj_dirs):
    if fix == 's':
        with h5py.File("libra_data/" + d + "/mem_data.hdf", 'r') as f:
            ...
            ps = f["p/data"][:, 0, 0]
            p = f["p/data"][:,0,:]
        #print(f'net momentum initial: {np.sum(p[0,:])}')
        #if (ps[0]>0.0 and s[-1]<0.0):
        #    print(f'forwards, then reverse: {d}')
        #if (ps[0]<0.0):
        #    print(f'Yeah negative allowed {d}')
        #if (s[-1]>0.0):
        #    print(f'Yeah positive s allowed {d}')
        #if (ps[0]<0.0 and s[-1]>0.0):
        #    print(f'reverse, then forwards: {d}')
