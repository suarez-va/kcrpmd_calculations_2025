...
if not current_dir.startswith("_sys_"):
...
indices = (np.arange(1,9) * np.floor(len(time) / 9)).astype(int)
...
ax1.errorbar(time[indices], kappa_avg[indices], np.absolute(kappa_se[indices]), fmt='none', ecolor='k', elinewidth=1)
